...
- [ ] T001 [US1] Create `internal/quality/` package with package doc
  in `quality.go`
- [ ] T002 [US1] Define `AssertionMapping` struct in
...
- [ ] T006 [US1] Define `PackageSummary` struct in
...
- [ ] T007 [US1] Define `Options` struct and `DefaultOptions()` in
...
- [ ] T008 [US1] Define `Assess()` function signature in
...
- [ ] T009 [US1] Add JSON tags to all new structs in
...
- [ ] T010 [US1] Implement `FindTestFunctions()` in
...
- [ ] T013 [US1] Handle multi-target case in `InferTarget()` —
...
- [ ] T017 [US1] Define `AssertionSite` struct in
...
- [ ] T023 [US1] Implement `t.Run` sub-test detection — detect
  `t.Run("name", func(t *testing.T) { ... })` closures and
  include their assertions at depth 0 (inlined)
- [ ] T024 [US1] Compute AssertionDetectionConfidence — ratio of
...
- [ ] T025 [P] [US1] Write assertion detection tests in
...
- [ ] T026 [US1] Implement `FindTargetCall()` in
  ...
  and method calls
- [ ] T027 [US1] Implement `TraceReturnValues()` in
...
- [ ] T028 [US1] Implement `TraceMutations()` in
...
- [ ] T029 [US1] Implement `MapAssertionsToEffects()` in
...
- [ ] T032 [US1] Handle discarded returns — detect `_ = target()`
  ...
  unasserted
- [ ] T033 [US1] Handle helper calls in SSA — when an assertion
...
- [ ] T034 [P] [US1] Write SSA mapping tests in
...
- [ ] T035 [US1] Implement `ComputeContractCoverage()` in
...
- [ ] T038 [US2] Implement suggestion generation — map side effect
...
- [ ] T039 [US1] Implement `BuildQualityReport()` in
  ...
  test-target pair
- [ ] T040 [US1] Implement `BuildPackageSummary()` in
...
- [ ] T041 [P] [US1] Write coverage computation tests in
...
- [ ] T042 [P] [US2] Write over-specification tests in
...
- [ ] T048 [US4] Add `--min-contract-coverage` flag — int, 0-100;
  when set and coverage is below threshold, return error (exit 1)
- [ ] T049 [US4] Add `--max-over-specification` flag — int;
  when set and over-specification count exceeds threshold, return
  error (exit 1)
- [ ] T050 [US4] Implement `runQuality()` in `cmd/gaze/main.go` —
  ...
  stdout/stderr
- [ ] T051 [US4] Implement threshold checking — report CI summary
...
- [ ] T053 [P] [US1] Create `internal/quality/testdata/src/welltested/`
  ...
  coverage)
- [ ] T054 [P] [US1] Create `internal/quality/testdata/src/undertested/`
  fixture — functions with known gaps: some contractual effects
  have no test assertions
- [ ] T055 [P] [US2] Create `internal/quality/testdata/src/overspecd/`
  fixture — tests that assert on incidental side effects (log
  output, goroutine spawn)
- [ ] T056 [P] [US1] Create `internal/quality/testdata/src/tabledriven/`
  fixture — table-driven tests with `t.Run` sub-tests; known
  assertion union
- [ ] T057 [P] [US1] Create `internal/quality/testdata/src/helpers/`
...
- [ ] T062 [US3] Write `TestSC004_Determinism` — run quality
...
- [ ] T064 [US1] Write `TestSC006_PackageSummary` — verify correct
