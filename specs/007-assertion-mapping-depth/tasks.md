...
- [x] T037 Verify SC-001: Run `gaze quality --format=json` across packages with tests and compute the weighted average contract coverage — target >= 80% per SC-001 [DEFERRED: requires built binary and real package analysis; completed during Spec 008 post-merge validation]
...
- [x] T044 Run quickstart.md validation steps: mapping accuracy test, quality on multilib fixture, before/after on real packages (crap, classify, docscan), confidence values in JSON output [DEFERRED: requires built binary and real package analysis; completed during Spec 008 post-merge validation]
