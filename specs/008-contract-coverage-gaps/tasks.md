...
- [x] T001 Build gaze binary with `go build ./cmd/gaze`
- [x] T002 Record baseline weighted average contract coverage by running `gaze quality --format=json ./...` and extracting `.summary.weighted_average_contract_coverage` — **Baseline: avg_contract_coverage=69.7%**
- [x] T003 Record baseline GazeCRAPload by running `gaze crap --format=json ./...` and extracting `.summary.gazecrapload` — **Baseline: gaze_crapload=11**
- [x] T004 Record baseline quadrant positions for target functions (`AnalyzeP1Effects`, `AnalyzeP2Effects`, `AnalyzeReturns`) by running `gaze crap --format=json ./...` — **P1Effects=Q4(1056), P2Effects=Q4(342), Returns=Q3(132), Visibility=Q3(182), Godoc=Q3(90)**
- [x] T005 Run spec 007 quickstart.md validation: `go test -race -count=1 -run TestSC003_MappingAccuracy ./internal/quality/...` (FR-021, completing deferred T044) — **PASS**
...
- [x] T006 Verify classify test fixtures are loadable: confirm `loadTestPackages(t)` in `internal/classify/classify_test.go` loads `contracts`, `callers`, `incidental`, and `ambiguous` fixtures without errors — **PASS**
- [x] T007 Verify analysis test fixtures are loadable: confirm `loadTestPackage(t, "p1effects")`, `loadTestPackage(t, "p2effects")`, and `loadTestPackage(t, "returns")` all succeed via `cachedTestPackage` in `internal/analysis/analysis_test.go` — **PASS**
- [x] T008 Verify full test suite passes before changes: run `go test -race -count=1 -short ./...` — **PASS (all 11 packages)**
...
- [x] T009 [P] [US1] Create `internal/classify/visibility_test.go` with table-driven tests for `AnalyzeVisibilitySignal` covering: exported function (+8 weight), exported return type (+6), exported receiver type (+6), weight clamp to maxVisibilityWeight (20), and reasoning string content for each dimension (FR-001, FR-002)
- [x] T010 [US1] Add edge case tests to `internal/classify/visibility_test.go` for: unexported functions (weight 0), nil `funcDecl` (zero signal), nil `funcObj` (zero signal) (FR-003)
- [x] T011 [P] [US1] Create `internal/classify/godoc_test.go` with table-driven tests for `AnalyzeGodocSignal` covering: each contractual keyword ("returns", "sets", "writes", "deletes", "updates", "modifies", "stores", "persists", "removes") paired with matching `effectType` (+15 weight), and non-matching effectType (0 weight). Note: "sends" and "creates" from FR-004 are intentionally absent from `godoc.go`'s keyword list; tests correctly verify weight=0 for these. (FR-004)
- [x] T012 [US1] Add incidental priority tests to `internal/classify/godoc_test.go` verifying: when godoc contains both "logs" and "returns", weight is -15 (incidental wins) (FR-005)
- [x] T013 [US1] Add edge case tests to `internal/classify/godoc_test.go` for: nil `funcDecl` (zero signal), nil `Doc` comment group (zero signal) (FR-006)
- [x] T014 [P] [US1] Create `internal/classify/callers_test.go` with tests for `AnalyzeCallerSignal` covering weight tiers: 1 caller -> 5, 2-3 callers -> 10, 4+ callers -> 15 (capped). Use `loadTestPackages(t)` to load `contracts`+`callers` fixtures and find target `types.Object` (FR-007)
- [x] T015 [US1] Add same-package caller exclusion test to `internal/classify/callers_test.go` verifying: callers from the same package are excluded from count (weight 0 if only same-package callers) (FR-008)
- [x] T016 [US1] Add edge case tests to `internal/classify/callers_test.go` for: nil `funcObj` (zero signal), functions with zero cross-package callers (zero signal) (FR-009)
- [x] T017 [US1] Run US1 verification: `go test -race -count=1 -run 'TestAnalyze(Visibility|Godoc|Caller)Signal' ./internal/classify/...`
...
- [x] T019 [US2] Add nil body edge case test to `internal/analysis/p1effects_test.go` verifying: `AnalyzeP1Effects` returns empty slice without panic when `fd.Body == nil` (edge case from spec)
...
- [x] T021 [US2] Add nil body edge case test to `internal/analysis/p2effects_test.go` verifying: `AnalyzeP2Effects` returns empty slice without panic when `fd.Body == nil`
...
- [x] T023 [US2] Verify Group B tests do not duplicate existing indirect tests: review `internal/analysis/analysis_test.go` to confirm new tests focus on direct invocation (`AnalyzeP1Effects`, `AnalyzeP2Effects`, `AnalyzeReturns`) rather than re-testing via `AnalyzeFunctionWithSSA` (FR-015)
- [x] T024 [US2] Run US2 verification: `go test -race -count=1 -run 'TestAnalyze(P1Effects|P2Effects|Returns)_Direct' ./internal/analysis/...`
...
- [x] T026 [P] [US3] Add description truncation test to `cmd/gaze/interactive_test.go` verifying: descriptions longer than 50 characters are truncated in output. Add empty results handling test (FR-017)
- [x] T027 [US3] Strengthen `TestBuildContractCoverageFunc_WelltestedPackage` in `cmd/gaze/main_test.go`: change `fn == nil` from acceptable to `t.Fatal` (assert `fn != nil`), assert `ok == true` for known function `welltested:Add`, assert `pct > 0` (not just `pct >= 0`). Keep `testing.Short()` guard. Note: the spec edge case "zero packages returns nil" is already covered by the existing `TestBuildContractCoverageFunc_InvalidPattern` test (FR-018, FR-019)
- [x] T028 [US3] Run US3 verification: `go test -race -count=1 -run 'Test(RenderAnalyzeContent|BuildContractCoverageFunc)' ./cmd/gaze/...`
...
- [x] T029 [US4] Rebuild gaze binary: `go build ./cmd/gaze`
- [x] T030 [US4] Re-measure weighted average contract coverage: `gaze quality --format=json ./...` and compare against Phase 1 baseline (FR-022). **After: 87.9% (was 69.7%) — IMPROVED**
- [x] T031 [US4] Re-run spec 007 quickstart.md validation: `go test -race -count=1 -run TestSC003_MappingAccuracy ./internal/quality/...` to verify no regressions (FR-021) — **PASS**
...
- [x] T032 [US5] Re-measure GazeCRAPload: `gaze crap --format=json ./...` and compare against Phase 1 baseline (FR-023). **After: 7 (was 11) — IMPROVED**
- [x] T033 [US5] Verify target functions exited Q4: `AnalyzeP1Effects` and `AnalyzeP2Effects` — **Contract coverage 0%→100%, GazeCRAP 1056→32 and 342→18. Still Q4 due to inherent complexity (32, 18) exceeding threshold (15), but GazeCRAP scores reduced by 95-97%. AnalyzeReturns, AnalyzeVisibilitySignal, AnalyzeGodocSignal all moved to Q1_Safe.**
- [x] T034 [US5] Verify `AnalyzeReturns` quadrant improvement: **Q3→Q1_Safe (GazeCRAP 132→11, contract coverage 0%→100%)**
...
- [x] T035 Run full test suite: `go test -race -count=1 -short ./...` to verify zero regressions (SC-005) — **PASS (all 11 packages)**
- [x] T036 Run linter: `golangci-lint run` to verify no lint violations introduced (SC-006) — **PASS (4 lint issues fixed: 3x S1009 nil-check simplification, 1x unused func removal; 1 pre-existing QF1005 in crap.go out of scope)**
- [x] T037 Verify GoDoc comments on all new test functions follow project conventions (GoDoc-style comments on exported test helpers if any) — **PASS (60/60 functions have GoDoc comments across 7 new test files)**
- [x] T038 Update `specs/008-contract-coverage-gaps/` artifacts: record final baseline/after measurements in a summary, mark spec tasks complete
- [x] T039 Assess documentation impact: check if README.md, AGENTS.md, or other docs need updates for any new patterns or conventions introduced — **No changes needed: test-only feature introduces no new commands, flags, output formats, architecture, conventions, or patterns**
